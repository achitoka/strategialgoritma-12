function showAttractionsDP() {
  var currentDay = document.getElementById("current-day").value;
  ...
  if (currentDay === "" || isNaN(money) || arrivalTime === "" || leftTime === "") {
      alert("Please fill in all fields.");
      return;
  }

  var arrivalHour = parseInt(arrivalTime.split(":")[0]);
  var arrivalMinute = parseInt(arrivalTime.split(":")[1]);
  var leftHour = parseInt(leftTime.split(":")[0]);
  var leftMinute = parseInt(leftTime.split(":")[1]);

  if (arrivalHour < 8 || arrivalHour > 21 || leftHour < 8 || leftHour > 22) {
      alert("Tokyo Disneyland is open from 8:00 AM until 10:00 PM");
      return;
  }

  var maxTime = (leftHour * 60 + leftMinute) - (arrivalHour * 60 + arrivalMinute);

  if (maxTime <= 0) {
      alert("Left time must be later than arrival time.");
      return;
  }
  ...
  var attractions = [
      ...
      { number: '26', name: "Swiss Family Treehouse", popularity: 6, price: 2500, duration: 15, daysOpen: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"], location: { x: 130, y: 125 } },
      { number: '30', name: "Western River Railroad", popularity: 8, price: 3500, duration: 25, daysOpen: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"], location: { x: 135, y: 130 } },
  ];
  ...
  if (resultsDiv) {
      ...
      // Filter attractions by current day and availability of money and time
      var availableAttractions = attractions.filter(function (attraction) {
          return attraction.daysOpen.includes(currentDay) && attraction.price <= money && attraction.duration <= maxTime;
      });

      if (availableAttractions.length > 0) {
          var list = document.createElement("ul");

          availableAttractions.forEach(function (attraction) {
              var listItem = document.createElement("li");
              listItem.textContent = attraction.name + " (Popularity: " + attraction.popularity + ", Price: " + attraction.price + ", Duration: " + attraction.duration + " mins)";
              list.appendChild(listItem);
          });

          resultsDiv.appendChild(list);
      } else {
          resultsDiv.textContent = "No attractions available with the given constraints.";
      }
  } else {
